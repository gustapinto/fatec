describe('Unit testing for Starwars API', (): void => {
    // Usa uma função async pois precisamos esperar (await) o método
    // testRequest.get(....)
    it('Should resource return a not null list of planets', async () => {
        // Testa uma requisição usando mocks
        const response = await global.testRequest.get('/planets')

        // Espera que a resposta não seja nula
        expect(response).not.toBeNull()
    })

    it('Should resource return a not empty list of planets', async () => {
        const response = await global.testRequest.get('/planets')
        // Obtém o corpo da response
        const body = response.body

        expect(body).not.toEqual([])
    })

    it('Should not resource return a empty object', async () => {
        const response = await global.testRequest.get('/planets')
        const body = response.body
    ...
    it('Should resource return a status code of 200', async () => {
        const response = await global.testRequest.get('/planets')
